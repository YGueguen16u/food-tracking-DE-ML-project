...
class TestCleanMealData(unittest.TestCase):
    ...
    def test_clean_meal_data(self):
        ...
        clean_meal_data(self.test_input_file, self.test_output_file, threshold_calories_high=1000, threshold_quantity_high=10)
        ...
        self.assertEqual(len(output_df), 2)  # Only rows with aliment_id not None should remain

        # Assert no null values in critical columns
        self.assertTrue(output_df[['aliment_id', 'quantity', 'Valeur calorique', 'Lipides', 'Glucides', 'Protein']].notnull().all().all())
